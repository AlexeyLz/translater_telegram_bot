    ...
    for entity in main_block:
        ...
        for def_and_sent_block in entity.find_all("div", {'class': 'def-block ddef_block'}):
            ...
            if found_definition_block is not None:
                ...
                if phrase_block is not None:
                    ...
                    if phrase_tags_section is not None:
                        ...
                        current_word_level, current_word_labels_and_codes, current_word_region, current_word_usage, current_word_domain = \
                            concatenate_tags(phrase_tags_section, current_word_level, current_word_labels_and_codes,
                                             current_word_region, current_word_usage, current_word_domain)
            ...
            update_word_dict(word_info, word=current_def_block_word, pos=pos, definition=definition,
                             alt_terms_list=m_alt_terms_list + alt_terms_list, sentences=sentences,
...
def get_info_word(word_data, word_result):
    ...
    desc_word = key
    try:
        desc_word += '\n---\nTranscription: ' + info['UK_IPA'][0]
    except:
        pass
    try:
        desc_word += '\n---\nAudio: ' + info['UK_audio_link']
    except:
        pass
    try:
